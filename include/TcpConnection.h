...
/*enable_shared_from_this是 C++ 标准库中的一个模板类，用于解决在类内部需要获取指向自身的 shared_ptr的问题*/
...
public:
    ...
    void sendFile(int fileDescriptor, off_t offest, size_t count);
...
private:
    ...
    // Socket Channel 和Acceptor类似    Acceptor => mainloop    TcpConnection => subloop
